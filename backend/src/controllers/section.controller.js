...
const ctrl = {
  ...
  async create(req, res, next) {
    try {
      ...
      await audit(req.user.id, 'CREATE', 'section', item.id, req.body, req);
  ...
  async update(req, res, next) {
    try {
      ...
      await audit(req.user.id, 'UPDATE', 'section', req.params.id, req.body, req);
      success(res, item);
    } catch (err) { next(err); }
  },
  async remove(req, res, next) {
    try {
      ...
      await audit(req.user.id, 'DELETE', 'section', req.params.id, null, req);
      success(res, {}, 'Deleted');
    } catch (err) { next(err); }
  },
};
