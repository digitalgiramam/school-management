...
const ctrl = {
  ...
  async create(req, res, next) {
    try {
      ...
      await audit(req.user.id, 'CREATE', 'class', item.id, req.body, req);
  ...
  async update(req, res, next) {
    try {
      ...
      await audit(req.user.id, 'UPDATE', 'class', req.params.id, req.body, req);
      success(res, item);
    } catch (err) { next(err); }
  },
  async remove(req, res, next) {
    try {
      ...
      await audit(req.user.id, 'DELETE', 'class', req.params.id, null, req);
      success(res, {}, 'Deleted');
    } catch (err) { next(err); }
  },
};
